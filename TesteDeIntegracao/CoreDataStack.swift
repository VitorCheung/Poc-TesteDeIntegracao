...
//  TesteDeIntegracao
//
//  Created by Vitor Cheung on 01/09/22.
//
...
final class CoreDataStack: ObservableObject {
  ...
  lazy var persistentContainer: NSPersistentCloudKitContainer = {
    let container = NSPersistentCloudKitContainer(name: "TesteDeIntegracao")
...
extension CoreDataStack {
  ...
  func delete(_ prof: Professor) {
    context.perform {
      self.context.delete(prof)
      self.save()
    }
  }
}
...
extension CoreDataStack {
  ...
  func getShare(_ prof: Professor) -> CKShare? {
    guard isShared(object: prof) else { return nil }
    guard let shareDictionary = try?
        persistentContainer.fetchShares(matching: [prof.objectID]),
      let share = shareDictionary[prof.objectID] else {
      print("Unable to get CKShare")
      return nil
    }
      share[CKShare.SystemFieldKey.title] = prof.nome
    return share
  }
    
  func createShare(_ prof: Professor) async throws -> CKShare?{
      do {
        let (_, share, _) = try await persistentContainer.share([prof], to: nil)
        share[CKShare.SystemFieldKey.title] = prof.nome
        return share
      } catch {
